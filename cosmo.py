...
#TO ENABLE MAILING FUNCTION YOU HAVE TO ENTER YOUR EMAIL ID AND PASSWORD FIRST IN THE CODE ITSELF
engine = pyttsx.init()
def g_crawler(query):
    ...
    engine.runAndWait()
    if dec == "yes":
        download_web_image(image_link, query )


def download_web_image(url,query):
    f_name = query + '.jpg'
    request.urlretrieve(url, f_name)
    g_image_viewer(f_name)

def tour_around(query):
...
def setreminder(minute,reminder):
    minutes = int(minute)
    print(minutes)
    Freq = 2500
    Dur = 1000
